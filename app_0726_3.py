...
# --- OpenStreetMap Nominatim API の設定 ---
NOMINATIM_API_URL = "https://nominatim.openstreetmap.org/search"
# Nominatimの利用ポリシーにより、User-Agentヘッダーは必須です。
# あなたのアプリ名と連絡先（メールアドレスなど）を記述してください。
NOMINATIM_USER_AGENT = "LaundryApp/1.0 kuroryu03@gmail.com)" 
...
# --- ジオコーディング関数 (OpenStreetMap Nominatim API) ---
def get_coordinates_from_nominatim(place_name):
    """
    場所名や郵便番号から緯度経度を取得する。
    Nominatimの利用ポリシーにより、User-Agentヘッダーは必須。
    """
    params = {
        "q": place_name,
        "format": "json",
        "limit": 1, # 最も関連性の高い結果を1つだけ取得
        "addressdetails": 0, # 詳細な住所情報は不要
    ...
    try:
        ...
        if results:
            ...
            st.warning(f"'{place_name}' の緯度経度が見つかりませんでした。より具体的な場所名や郵便番号を試してください。")
            return None, None
    except requests.exceptions.RequestException as e:
        st.error(f"ジオコーディングAPIの呼び出し中にエラーが発生しました: {e}")
        return None, None
...
if uploaded_file is not None:
    ...
    if show_weather_section_based_on_detection:
        ...
        if display_weather_option == "はい":
            ...
            # --- 新しいジオコーディング入力セクション ---
            st.markdown("##### 場所名や郵便番号から緯度経度を検索")
            place_input = st.text_input("場所名または郵便番号を入力 (例: 東京都庁, 163-8001)", key="place_search_input")
            
            if st.button("緯度経度を検索", key="search_coords_button"):
                if place_input:
                    with st.spinner(f"'{place_input}' の緯度経度を検索中..."):
                        found_lat, found_lon = get_coordinates_from_nominatim(place_input)
                        if found_lat is not None and found_lon is not None:
                            # 検索結果をセッションステートに保存し、緯度経度入力フィールドを更新
                            st.session_state.latitude_input = str(found_lat)
                            st.session_state.longitude_input = str(found_lon)
                            st.session_state.last_selected_city = "その他（緯度経度入力）" # 選択都市をリセット
                            st.success(f"'{place_input}' の緯度経度が見つかりました: 緯度 {found_lat:.4f}, 経度 {found_lon:.4f}")
                            st.rerun() # UIを更新するために再実行 (st.experimental_rerun() の代わりに)
                        # else: get_coordinates_from_nominatim関数内でエラーメッセージが表示される
                else:
                    st.warning("場所名または郵便番号を入力してください。")
            
            st.markdown("---") # 区切り線
            st.markdown("##### または、リストから選択するか直接入力")
            # --- 既存の都市選択と緯度経度自動入力部分 ---
            ...
            if selected_city_name and selected_city_name != "その他（緯度経度入力）":
                latitude_from_city = CITY_COORDINATES[selected_city_name]["latitude"]
                longitude_from_city = CITY_COORDINATES[selected_city_name]["longitude"]
                
                # ジオコーディング検索結果がない場合のみ、都市選択の結果を反映
                # ジオコーディング検索で更新された場合は、都市選択で上書きしないようにする
                if 'last_selected_city' not in st.session_state or st.session_state.last_selected_city != selected_city_name:
                    st.session_state.latitude_input = str(latitude_from_city) if latitude_from_city is not None else ""
                    st.session_state.longitude_input = str(longitude_from_city) if longitude_from_city is not None else ""

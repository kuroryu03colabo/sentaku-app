...
def get_location_from_ip():
    """IPアドレスからおおよその都市名を取得します。"""
    try:
        # ip-api.com を利用してIPアドレスから位置情報を取得
        # lang=ja を追加して、可能な限り日本語の都市名を取得
        response = requests.get("http://ip-api.com/json/")
        response.raise_for_status() # HTTPエラーがあれば例外を発生させる
        data = response.json()
        if data and data.get('status') == 'success':
            return data.get('city', '東京') # 都市名が見つからない場合は「東京」をデフォルトとする
        else:
            return '東京' # 取得失敗時も「東京」をデフォルトとする
    except requests.exceptions.RequestException as e:
        # エラーが発生した場合はコンソールにメッセージを出力し、デフォルトを返す
        print(f"IPアドレスからの位置情報取得中にエラーが発生しました: {e}")
        return '東京'
...
if uploaded_file is not None:
    ...
    if display_weather_option == "はい":
        ...
        st.info("※お使いのIPアドレスからおおよその位置情報を自動取得しています。")
        
        # IPアドレスから取得した都市名を初期値として設定
        initial_city = get_location_from_ip()
        city_name = st.text_input(
            "天気予報を表示したい都市名を入力してください (例: 東京):",
            value=initial_city, # 初期値をIPから取得した都市名に設定
            key="city_input"
        )
        
        if st.button("天気予報を取得", key="get_weather_button"):

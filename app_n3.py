...
def get_weather_by_zip(zip_code, country_code, api_key):
    """指定された郵便番号と国コードの天気情報をOpenWeatherMap APIから取得します。"""
    params = {
        "zip": f"{zip_code},{country_code}", # 郵便番号と国コードをカンマ区切りで指定
        "appid": api_key,
        "units": "metric", # 摂氏で取得
        "lang": "ja" # 日本語で取得
    }
...
if show_weather_section_based_on_detection:
    ...
    if display_weather_option == "はい":
        ...
        # ユーザーに郵便番号を入力してもらう
        zip_code = st.text_input("天気予報を表示したい郵便番号を入力してください (例: 100-0001):", key="zip_input")
        
        if st.button("天気予報を取得", key="get_weather_button") and zip_code:
            if OPENWEATHER_API_KEY == "YOUR_OPENWEATHER_API_KEY":
                st.warning("OpenWeatherMap APIキーを設定してください。")
            else:
                with st.spinner("天気情報を取得中..."):
                    # 日本の国コード 'JP' を指定
                    weather_data = get_weather_by_zip(zip_code, "JP", OPENWEATHER_API_KEY)
                    
                    if weather_data:
                        # 郵便番号からの取得の場合、都市名が 'name' フィールドにある
                        ...
                        st.error("指定された郵便番号の天気情報を取得できませんでした。郵便番号が正しいか確認してください。")
        elif st.button("天気予報を取得", key="get_weather_button_no_zip") and not zip_code:
